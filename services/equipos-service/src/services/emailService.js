...
async function generarActaDevolucion(itemData, returnEmail) {
    try {
        const fecha = new Date().toLocaleDateString('es-ES');
        ...
        const actaContent = `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Acta de Devolución - Equipo ${itemData.serial}</title>
                <style>
                    body { font-family: Arial, sans-serif; margin: 20px; }
                    .header { background-color: #f0f0f0; padding: 20px; text-align: center; }
                    .content { margin: 20px 0; }
            ...
            <body>
                ...
                <div class="content">
                    <h2>Información del Equipo</h2>
                    <div class="equipment-info">
                        <p><strong>Serial:</strong> ${itemData.serial}</p>
                        <p><strong>Nombre:</strong> ${itemData.nombre || 'N/A'}</p>
                        <p><strong>Marca:</strong> ${itemData.marca || 'N/A'}</p>
                        <p><strong>Modelo:</strong> ${itemData.modelo || 'N/A'}</p>
                    </div>
                    
                    <h2>Información de Devolución</h2>
                    <div class="equipment-info">
                        <p><strong>Agente:</strong> ${itemData.agente_nombre || 'N/A'}</p>
                        <p><strong>Fecha de Devolución:</strong> ${fecha}</p>
                        <p><strong>Estado del Equipo:</strong> [Verificar estado]</p>
                    </div>
        ...
        const mailOptions = {
            from: process.env.SMTP_USER,
            to: returnEmail,
            subject: `Acta de Devolución - Equipo ${itemData.serial}`,
            html: actaContent
        };

        await transporter.sendMail(mailOptions);
        console.log(`✅ Acta de devolución enviada a ${returnEmail}`);

    } catch (error) {
        console.error('❌ Error al enviar acta de devolución:', error);
        throw error;
    }
}
